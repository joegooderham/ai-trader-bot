...
| BACKLOG-036 | Desktop widget remote access | Allow the desktop P&L widget (`tools/desktop_widget/`) to work from any device, not just localhost. Currently blocked by Cloudflare Access OAuth (302 redirect). Options: Cloudflare Access Service Token (client ID + secret in widget `.env`), or a lightweight API key bypass route in the dashboard. Also needs a health indicator in the widget when auth fails so it's obvious data is stale. |
| BACKLOG-037 | Desktop widget P&L accuracy | Widget total P&L can drift from dashboard if a refresh fails silently. Needs: visual staleness warning (e.g. amber dot) if last successful fetch is >60s old, error handling that surfaces connection/auth failures to the user, and verify the P&L calculation matches the dashboard exactly (both should use the same `/api/cmd/positions` `unrealizedPL` field from IG live). |
